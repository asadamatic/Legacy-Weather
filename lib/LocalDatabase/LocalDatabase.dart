...
class LocalDatabaseCity {
  LocalDatabaseCity();

  static const databaseName = 'tablecity.db';
  ...
  initializeDB() async {
    return await openDatabase(join(await getDatabasesPath(), databaseName),
      ...
      return await db.execute(
          'CREATE TABLE tablecity(id INTEGER PRIMARY KEY, name TEXT)');
    });
  }
  ...
  Future<void> insertData(City city) async {
    final Database db = await database;
    db.insert('tablecity', city.toMap(), conflictAlgorithm: ConflictAlgorithm.ignore);
  }

  Future<void> update(City city) async {
    final Database db = await database;
    db.update('tablecity', city.toMap(), where: 'id = ?', whereArgs: [city.id]);
  }
